...
const bucket = dev ? 'store-house' : 'store-house-prod'

const operator = dev ? 'storehouse' : 'storehouseprod'

const remotePath = '/{year}-{mon}-{day}/files/{random32}{.suffix}'

const cname = dev ? 'https://storehouse-upyun.chuanshui.cn' : 'https://storehouse-upyun.chuanshui.com'

const getSignatureUrl = config.apiUrl + 'upyun-signature-callback'
...
module.exports = {
  upload: function (localPath, quality = 100) {
    let compressImageP
    if (quality == 100) {
      //不压缩
      compressImageP = Promise.resolve(localPath)
    } else {
      //压缩图片
      compressImageP = new Promise((resolve, reject) => {
        wx.compressImage({
          src: localPath,
          quality: quality,
          success: function (res) {
    ...
    return compressImageP.then(path => {
      return new Promise((resolve, reject) => {
        console.log("compressImage", path)
        upyun.upload({
          localPath: path,
